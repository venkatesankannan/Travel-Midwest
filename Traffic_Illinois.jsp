   ...
   $('#expy').change(function() {
 	...
 	if((t1.val() == 'travelTime')) {
 		...
 		//alert(expy);
 				  ...
 				  $.post("./traffic_Rservlet.jsp" ,{'expy': expy,'hid1':hid1},  
	...
	if((t1.val() == 'congestionData')) {
 		
		 $("#congestion_init").hide();
		var expy=$('#expy').val();
		var hid1=$('#hid1').val();
		//alert("before");
		//alert(add);
				  ...
				  $.post("./congestion_Rservlet.jsp" ,{'expy': expy,'hid1':hid1},  
	...
	if((t1.val() == 'incidentData')) {
		...
		//alert(expy);
				  ...
				  $.post("./incident_Rservlet.jsp" ,{'expy': expy},  
...
<li>
<a href="Traffic_Illinois.jsp">Travel Report-Chicago</a>
</li>
</ul>
</div>
    <!-- breadcrumb ends -->
...
<%
	String report=request.getParameter("optionsRadios");%>
	<input type="hidden" value=<%=report %> id="hid" name="hid">
	...
	%>
	<input type="hidden" value=<%=arr[0]%> id="hid1" name="hid1">
	<%
	if(report.equals("congestionData"))
	{
		try {
			...
			String from=null;
			String to=null;
			String myQuery = "SELECT * FROM \"congestionData\" where expressway='"+exp+"'";
			...
			myResultSet = myPreparedStatement.executeQuery();%>
						   	<div id="congestion_init" class="row" >
...
&nbsp;<a href="Home.jsp" class="btn btn-primary" ><i class="glyphicon glyphicon-chevron-left glyphicon-white"></i> Back </a>
	...
	if(report.equals("incidentData"))
	{
		try {
			...
			//double speed;
			//int traveltime;
			String myQuery = "SELECT * FROM \"incidentData\" where expressway='"+exp+"'";
			Connection myConnection=null;
			PreparedStatement myPreparedStatement=null;
			ResultSet myResultSet = null;
			Class.forName(driver);
...
&nbsp;<a href="Home.jsp" class="btn btn-primary" ><i class="glyphicon glyphicon-chevron-left glyphicon-white"></i> Back </a>
	...
	if(report.equals("travelTime"))
	{
		try {
			...
			String myQuery = "SELECT * FROM \"travelTime\" where expressway='"+exp+"'";
			Connection myConnection=null;
			PreparedStatement myPreparedStatement=null;
			ResultSet myResultSet = null;
			Class.forName(driver);
